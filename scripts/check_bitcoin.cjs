...
async function checkBitcoin() {
    ...
    assetsSnap.forEach(doc => {
        ...
        if (id.toLowerCase().includes('bitcoin') ||
    ...
    txSnap.forEach(doc => {
        ...
        if (assetId.toLowerCase().includes('bitcoin') ||
            assetId.toLowerCase().includes('btc')) {
    ...
    if (bitcoinTxs.length > 0) {
        ...
        const totalInvested = bitcoinTxs
            .filter(tx => ['Plan', 'Aportación', 'Retirada', 'Dividendo', 'Traspaso', 'Venta'].includes(tx.type))
        ...
        Array.from(uniqueAssetIds).sort().forEach(id => {
            console.log(`  - "${id}"`);
        });
    }
