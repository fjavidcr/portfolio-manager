...
const getInvested = (assetId: string) => {
    if (!$portfolio.value.transactions) return 0;
    
    // Logic from portfolioStore (sum of valid types)
    const validTypes = ['Plan', 'Aportación', 'Retirada', 'Dividendo', 'Traspaso', 'Venta']; 
    
    return $portfolio.value.transactions
        .filter(tx => tx.assetId === assetId && validTypes.includes(tx.type))
