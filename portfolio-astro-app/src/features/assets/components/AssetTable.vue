  ...
  <div class="space-y-6">
    ...
    <div
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <!-- Type Filter -->
        <div>
          <label for="type-filter" class="block text-xs font-medium text-secondary mb-2">Type</label>
        ...
        <!-- Platform Filter -->
        <div>
          <label for="platform-filter" class="block text-xs font-medium text-secondary mb-2">Platform</label>
          <select id="platform-filter" v-model="selectedPlatform"
            class="block w-full px-3 py-2 bg-surface border border-outline-variant rounded-lg text-on-surface focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary sm:text-sm">
            <option value="">All Platforms</option>
            <option v-for="platform in $portfolio.platforms" :key="platform.id" :value="platform.id">
        ...
        <div class="flex items-end">
          <button @click="searchQuery = ''; selectedType = ''; selectedPlatform = ''"
    ...
    <!-- Empty State -->
    <div v-else-if="activeAssets.length === 0 && archivedAssets.length === 0" class="text-center py-10 text-secondary">
      No assets found.
        ...
        <div v-for="asset in activeAssets" :key="asset.id"
          ...
          <div class="flex justify-between items-start mb-4">
            <div class="min-w-0 flex-1 mr-2">
              <h3 class="text-lg font-semibold text-on-surface truncate" :title="asset.name">{{ asset.name
                }}</h3>
              <p class="text-xs text-secondary font-medium tracking-wide truncate" :title="asset.id">{{
                asset.id }}</p>
              <div class="mt-2 flex gap-2 flex-wrap">
                <span v-if="asset.platformId"
            ...
            <div class="flex gap-1">
              <button @click="toggleArchive(asset)"
          ...
          <div class="mb-4">
            ...
            <p class="text-3xl font-bold text-on-surface tracking-tight">{{
              formatCurrency(asset.currentValue) }}</p>
          ...
          <div class="grid grid-cols-2 gap-y-3 gap-x-2 border-t border-outline-variant pt-3 text-sm">
            ...
            <div>
              ...
              <p class="font-bold"
                :class="getProfit(asset) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
            ...
            <div class="text-right">
              <p class="text-[10px] text-secondary uppercase font-bold">ROI %</p>
              <p class="font-bold"
                :class="getRoiPercent(asset) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
      ...
      <div v-if="archivedAssets.length > 0" class="mt-8">
        ...
        <button @click="showArchived = !showArchived"
          ...
          <div class="flex items-center gap-3">
            <svg class="w-5 h-5 text-secondary transition-transform" :class="{ 'rotate-90': showArchived }" fill="none"
        ...
        <div v-show="showArchived" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 opacity-75">
          <div v-for="asset in archivedAssets" :key="asset.id"
            ...
            <div class="grid grid-cols-2 gap-y-3 gap-x-2 border-t border-outline-variant pt-3 text-sm">
              ...
              <div>
                <p class="text-[10px] text-secondary uppercase font-bold">Invertido</p>
                <p class="font-medium text-on-surface/80">{{ formatCurrency(getInvested(asset.id)) }}
                </p>
              </div>
