...
import { dcaStore, dcaStatus, dcaItemsWithAssets, updatePlanValues, addPlanItem, deletePlanItem } from '../stores/dcaStore'
...
const goalOptions = vueComputed(() => {
    return dca.value.goals.map(goal => ({
        id: goal.id,
        label: goal.name
    } as SelectOption))
})
...
<template>
  ...
  <div v-else class="bg-surface rounded-2xl shadow-sm border border-outline-variant/30">
      ...
      <table class="table w-full">
        <thead>
          <tr class="text-secondary border-b border-outline-variant/30">
            <th class="bg-transparent font-bold text-[10px] uppercase tracking-widest pl-6 py-4">Activo</th>
            <th class="bg-transparent font-bold text-[10px] uppercase tracking-widest py-4">Plataforma</th>
            <th class="bg-transparent font-bold text-[10px] uppercase tracking-widest py-4">Objetivo</th>
            <th class="bg-transparent font-bold text-[10px] uppercase tracking-widest text-right py-4">Peso %</th>
            <th class="bg-transparent font-bold text-[10px] uppercase tracking-widest text-right pr-6 py-4">Inversión</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-outline-variant/20">
          <tr v-for="item in itemsWithAssets" :key="item.id" class="hover:bg-surface-container-low transition-colors border-b border-outline-variant/10 group">
            <td class="pl-6 py-4">
            ...
            <td class="py-4">
                <span class="badge badge-sm badge-outline font-bold text-[10px] border-outline-variant/50 text-secondary">
                    {{ item.platformName }}
                </span>
            </td>
            <td class="py-4">
                <span class="text-xs font-bold text-on-surface-variant">{{ getGoalName(item.goalId) }}</span>
            </td>
            <td class="text-right py-4">
                <div class="flex items-center justify-end gap-1">
                    <input 
                        ...
                        @input="e => handleItemPercentageChange(item.id, (e.target as HTMLInputElement).value)"
    ...
    <div v-if="itemsWithAssets.length === 0" class="p-12 text-center">
        <p class="text-sm font-medium text-secondary italic">No hay activos configurados para la planificación.</p>
    </div>
                    ...
                    <template #option="{ option }">
                        <div class="flex items-center justify-between gap-2">
                            <div>
                                ...
                                <p class="text-[10px] text-secondary font-medium">{{ option.metadata?.ticker }}</p>
                            </div>
                            <span class="text-[9px] font-black uppercase text-secondary/40 border border-outline-variant/30 px-1.5 py-0.5 rounded">
